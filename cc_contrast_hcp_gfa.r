...
data<-read.csv("/Users/dgari004/Google\ Drive/CC_RDI_NODDI/new\ contrast\ analyses/gfa_hcp840_within_subjects.csv")
attach(data)

#make plot. Adjust fo
cc.plot <- function(data){
theme_set(theme_light(base_size = 16))
p<-ggplot(mapping = aes(x = cc_region, y = GFA)) +
    ...
    stat_summary(fun.y = mean, geom="line") + ylab('GFA') +
    scale_x_discrete('Corpus Callosum Region', waiver(), labels = c("G1", "G2", "G3", "B1", "B2", "B3", "I", "S1", "S2", "S3"), c(0:9)) +
    ggtitle('GFA Values by Corpus Callosum Region') + theme(panel.border = element_blank(),
...
tiff("GFA_hcp840.tiff", units = 'in', width = 6, height = 8, res = 200, compression = "lzw")
cc.plot(data)
dev.off()
...
mGFA1 <- lmer(GFA ~ cc_region + (1 | subject),  data = data, na.action = na.omit, REML=TRUE)
summary(mGFA1)
tapply(GFA, cc_region, mean, na.rm=TRUE) #summarize means of each condition
n<-tapply(GFA, cc_region, length)[1] #summarize n of each condition
means = emmeans(mGFA1, "cc_region") #complete summary with standard error estimates
